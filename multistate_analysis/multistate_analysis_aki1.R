################################################################################
######      Multi-State Analysis of AKI Recovery (Stage 1 at Onset)       ######
################################################################################
...
colnames(df) <- c("ENCOUNTERID", 
                  ...
                  "BMI","HYPERTENSION", "ONSET_SINCE_ADMIT", 
                  "ANION_GAP", "ALBUMIN", "ALP", "AST", "BILIRUBIN", "BNP", 
                  "CALCIUM", "CARBON_DIOXIDE", "GLUCOSE", "LACTATE", 
                  "LYMPHO_LEUKO_RATIO", "Potassium", "PT", 
                  "site"
                  )
...
for (site_name in names(model_datasets)){
  if (site_name =='Site1'){
    cox_formula = as.formula(paste('Surv(tstart, tstop, EVENT_STATUS) ~ ', 
                                   paste(paste(covs[!covs %in% c("CARBON_DIOXIDE", "PT")], 
                                               collapse = ":TRANS + "), 
                                         ':TRANS'), '+strata(TRANS)'))
  } else if (site_name =='Site2'){
    cox_formula = as.formula(paste('Surv(tstart, tstop, EVENT_STATUS) ~ ', 
                                   paste(paste(covs[!covs %in% c("ANION_GAP", "BNP")], 
                                               collapse = ":TRANS + "), 
                                         ':TRANS'), '+strata(TRANS)'))
  } else if (site_name =='Site3'){
    cox_formula = as.formula(paste('Surv(tstart, tstop, EVENT_STATUS) ~ ', 
                                   paste(paste(covs[!covs %in% c("LYMPHO_LEUKO_RATIO")], 
                                               collapse = ":TRANS + "), 
                                         ':TRANS'), '+strata(TRANS)'))
  } else{
    cox_formula = as.formula(paste('Surv(tstart, tstop, EVENT_STATUS) ~ ', 
...
var_common = c('SCR_BASELINE', 'SCR_FD', 'SCR_MEAN', 
               'SYSTOLIC_FD', 'SYSTOLIC_MEAN', 
               'ALBUMIN', 'BILIRUBIN')
var_name_list <- c('SCR_BASELINE' = 'SCr (baseline)', 
                   ...
                   'GLUCOSE'= 'Glucose',
                   'LACTATE' = 'Lactate',
                   'LYMPHO_LEUKO_RATIO'=  'Lympho/(100*Leuko)',
                   'Potassium' = 'Potassium',
                   'PT' = "PT"
                   )
trans_name_list <- c(
...
variables_group_2 <- names(var_name_list)[6:11]
variables_group_3 <- names(var_name_list)[12:17]
variables_group_4 <- names(var_name_list)[18:23]
...
png(paste0(base_path,"/result/plot_mscox_aki1.png"), 
    width = 12, 
    height = 10, 
    units = "in", 
    res = 300)
...
png(paste0(base_path,"/result/plot_basehaz_aki1.png"), 
    width = 11, 
    height = 8, 
    units = "in", 
    res = 300)
